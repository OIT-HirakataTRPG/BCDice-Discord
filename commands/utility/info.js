...
module.exports = {
	...
	async execute(interaction) {
		...
		if (system !== "DiceBot") {
			dicehelpEmbed.setFields({
				name: `${await translateSystemName(system)} ダイス説明`,
				value: `>>> ${helpText.replace(/\*/g, "\\*")}`,
			});
			dicehelpEmbed.addFields({
				name: "共通ダイスの詳細は下記URLのコマンドガイドを参照",
				value: "https://docs.bcdice.org/",
			});
		} else {
			dicehelpEmbed.setFields({
				name: "共通ダイス",
				value: `>>> ${helpText.replace("詳細は下記URLのコマンドガイドを参照\nhttps://docs.bcdice.org/\n", "").replace(/^(.+)/gm, `- $1`)}`,
			});
			dicehelpEmbed.addFields({
				name: "詳細は下記URLのコマンドガイドを参照",
				value: "https://docs.bcdice.org/",
			});
		}
		await interaction.reply({
